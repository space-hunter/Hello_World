...
class IStreamFilmListeScreen(Screen):
	...
	def __init__(self, session, genreLink, genreName):
		...
		self["actions"]  = ActionMap(["OkCancelActions", "ShortcutActions", "ColorActions", "SetupActions", "NumberActions", "MenuActions", "EPGSelectActions","DirectionActions","InfobarEPGActions"], {
			"ok"    : self.keyOK,
			"cancel": self.keyCancel,
			"up" : self.keyUp,
			"down" : self.keyDown,
	...
	def ShowCoverFile(self, picPath):
		if fileExists(picPath):
			...
			print "eliinfo - ShowCover0 Size Nr + ", size.width(), size.height(), self.scale[0], self.scale[1]
	...
	def keyShowThumb(self):
		print "Funktion show Thumb Pictures"
		if self.keyLocked:
			return
		#self.session.open(ShowThumbScreen, self.filmListe)
		#drtuberUrl(self.session).getVideoUrl(x, y, z, self.gotVideoPage)
		#self.session.open(m2kGenreScreen, "default")
		(streamLink, streamName, imageLink) = self.session.openWithCallback(showThumb, self.filmListe)
		self.session.open(IStreamStreams, streamLink, streamName, imageLink)		
...
class IStreamStreams(Screen, ConfigListScreen):
	...
	def ShowCover(self, picData):
		...
		if fileExists("/tmp/Icon.jpg"):
			...
			self.picload = ePicLoad()
			size = self['coverArt'].instance.size()
			print "eliinfo - ShowCover2 Size Nr + ", size.width(), size.height(), self.scale[0], self.scale[1]
			self.picload.setPara((size.width(), size.height(), self.scale[0], self.scale[1], False, 1, "#FF000000"))
